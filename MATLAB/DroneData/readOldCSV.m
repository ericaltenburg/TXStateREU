function readOldCSV(num, startTime, endTime, AperV)
...
while true
    l = fgetl(fp);
    if ~ischar(l)% binlog = 'logs/1.log';
% tstart = 628; % start time
% tend = 1243;  % end time
        break;
    end;
    d = split(l,',');
...
save(['runData/',logNum,'.mat'], 'tstart', 'tend', 'dmode', 'dimu', 'dbaro', 'datt', 'dcurr', 'dctun', 'dntun', 'dgps');
